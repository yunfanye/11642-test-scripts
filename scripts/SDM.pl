...
my $and_weight = $ARGV[0];
my $near_weight = $ARGV[1];
my $window_weight = $ARGV[2];
my $input_path = $ARGV[3];
my $output_path = $ARGV[4];

open(my $out_fh, '>', $output_path);
open(my $in_fh, '<', $input_path);
while (!eof($in_fh)) {
    $line = readline($in_fh);
    my @fields = split /:/, $line;
    print $out_fh $fields[0].":"
        .formulate_query($fields[1], "sd",
        $and_weight, $near_weight, $window_weight ) . "\n";
}
